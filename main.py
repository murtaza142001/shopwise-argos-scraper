...
def dashboard():
    return '''
    <html>
        <head><title>Argos Scraper Dashboard</title></head>
...
def run_scraper():
    with sync_playwright() as p:
        ...
        # Simple scrape: get all links
        links = page.eval_on_selector_all("a", "elements => elements.map(el => el.href).filter(Boolean)")
        browser.close()
